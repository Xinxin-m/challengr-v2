    ...
    return (
      ...
      >
        <motion.button
          ...
          className={`
            relative w-full flex items-center space-x-4 p-4 rounded-2xl 
            transition-all duration-300 ease-in-out text-left group overflow-hidden
            ${isCollapsed ? 'justify-center px-3' : ''}
            ${isActive 
              ? `bg-gradient-to-r ${item.color} ${item.glow} shadow-2xl text-white border-2 border-white/30` 
              : 'text-white/70 hover:text-white hover:bg-white/10 border-2 border-transparent hover:border-white/30'
            }
          `}
        ...
        >
          {isActive && (
            <motion.div
              className={`absolute inset-0 bg-gradient-to-r ${item.color} opacity-20 rounded-2xl blur-2xl`}
              initial={{ scale: 0 }}
              animate={{ scale: 1.2 }}
          ...
          <div className={`relative z-10 ${isCollapsed ? '' : 'flex-shrink-0'}`}>
            ...
            >
              ...
              >
                <Icon className={`w-6 h-6 transition-all duration-300 ${isActive ? 'text-white' : 'text-white/70 group-hover:text-white'}`} />
            ...
            {!isCollapsed && (
              ...
              >
                <div className="flex items-center space-x-2">
                  <div className={`font-black text-lg whitespace-nowrap transition-all duration-300 ${isActive ? 'text-white' : 'text-white/90'}`}>
                    {item.label}
                  </div>
                </div>
          ...
          {hoveredItem === item.id && !isActive && (
            <motion.div
              className="absolute inset-0 bg-gradient-to-r from-white/5 via-white/10 to-white/5 rounded-2xl"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            />
      ...
      <div className="relative h-full">
        ...
        <div className="relative h-full flex flex-col p-4 overflow-hidden">
          <motion.div
            ...
            className="mb-6"
          >
            {/* Removed the toggle collapse button */}
            <AnimatePresence>
              {!isCollapsed && (
                <motion.div
                ...
                >
                  <motion.button
                    ...
                    className="w-full bg-gradient-to-br from-slate-800/60 via-indigo-800/40 to-purple-800/60 backdrop-blur-sm border border-cyan-400/30 rounded-2xl p-4 text-left transition-all duration-300 hover:border-cyan-400/60 hover:shadow-xl hover:shadow-cyan-500/20"
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                  >
                    <div className="flex items-center space-x-4 mb-4">
                      <div className="w-14 h-14 bg-gradient-to-br from-purple-500 via-blue-500 to-cyan-500 rounded-2xl flex items-center justify-center text-2xl relative overflow-hidden">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center space-x-2">
                          ...
                          <Badge className="bg-gradient-to-r from-blue-500/30 to-purple-500/30 text-blue-300 border-blue-400/50">
                            {userProgress.currentTier || 'Foundation'}
                          </Badge>
                        </div>
                        <p className="text-white/70 text-sm">Lv. {userProgress.level || '15'}</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="relative">
                        <Progress value={calculateCultivationProgress()} className="h-3 bg-slate-700/50" />
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="bg-gradient-to-br from-amber-500/20 to-yellow-500/20 backdrop-blur-sm border border-amber-400/40 rounded-xl p-3 text-center">
                      <Coins className="w-5 h-5 text-amber-400 mx-auto mb-1" />
                      <div className="text-amber-400 font-bold text-xs">2500</div>
                      <div className="text-white/80 text-xs">coins</div>
                    </div>
                    <div className="bg-gradient-to-br from-cyan-500/20 to-blue-500/20 backdrop-blur-sm border border-cyan-400/40 rounded-xl p-3 text-center">
                      <Zap className="w-5 h-5 text-cyan-400 mx-auto mb-1" />
                      <div className="text-amber-400 font-bold text-sm">12500</div>
                      <div className="text-white/80 text-xs">XP</div>
                    </div>
                    <div className="bg-gradient-to-br from-purple-500/20 to-pink-500/20 backdrop-blur-sm border border-purple-400/40 rounded-xl p-3 text-center">
                      <BadgeDollarSign className="w-5 h-5 text-purple-400 mx-auto mb-1" />
          ...
          {/* Navigation content - only scrollable on small screens */}
          <div className="flex-1 space-y-3 mb-6 overflow-y-auto overflow-x-hidden scrollbar-thin scrollbar-thumb-slate-600 scrollbar-track-transparent lg:scrollbar-none">
          ...
          >
            {isCollapsed ? (
              ...
              >
                <div className="relative">
                  <motion.button
                    ...
                    className="w-12 h-12 bg-gradient-to-r from-cyan-500 via-purple-600 to-amber-500 text-white rounded-2xl shadow-2xl hover:shadow-xl transition-all duration-300 flex items-center justify-center relative overflow-hidden"
              ...
              <div className="flex space-x-3">
                <motion.button
                  onClick={onCreatePost}
                  className="flex-1 bg-gradient-to-r from-cyan-500 via-purple-600 to-amber-500 text-white p-4 rounded-2xl font-black shadow-2xl hover:shadow-xl transition-all duration-300 flex items-center justify-center space-x-3 relative overflow-hidden"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-white/20 to-transparent" />
                ...
                <motion.button
                  ...
                  className="flex-1 bg-gradient-to-r from-red-500 via-orange-600 to-yellow-500 text-white p-4 rounded-2xl font-black shadow-2xl hover:shadow-xl transition-all duration-300 flex items-center justify-center space-x-3 relative overflow-hidden"
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-white/20 to-transparent" />
